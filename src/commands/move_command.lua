...
function MoveCommand:execute()
	local tilePos = board:toTilePos((self.dir * TILE_SIZE) + self.actor.position)
	if not self.actor.moving and board:check(self.actor.tilePos + self.dir) then
...
function MoveCommand:undo()
	...
	if not self.actor.moving and board:check(selgf.actor.tilePos - self.dir) then
		print("#commands before: "..commands.last + 1)
